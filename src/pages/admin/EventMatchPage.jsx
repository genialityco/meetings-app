...
Reglas: 1) No matches de misma empresa. 2) Score 0-1. 3) Max ${payload.maxPerUser || eventConfig?.maxMeetingsPerUser || 10} por comprador.`;
  ...
  const generateLocalMatches = async () => {
    const maxMeetingsPerUser = eventConfig?.maxMeetingsPerUser ?? 10;

    // Build set of existing meeting pairs to avoid duplicates
    const existingPairs = new Set();
    try {
      const meetingsSnap = await getDocs(collection(db, "events", eventId, "meetings"));
      meetingsSnap.docs.forEach((d) => {
        const md = d.data() || {};
        if (Array.isArray(md.participants) && md.participants.length >= 2) {
          const [a, b] = md.participants;
          existingPairs.add(`${a}_${b}`);
          existingPairs.add(`${b}_${a}`);
        } else if (md.requesterId && md.receiverId) {
          existingPairs.add(`${md.requesterId}_${md.receiverId}`);
          existingPairs.add(`${md.receiverId}_${md.requesterId}`);
        }
      });
    } catch (e) {
      console.warn("No se pudieron leer reuniones existentes, procediendo sin filtro:", e);
    }

    const results = compradores.map((comprador) => {
      const matches = vendedores
        .map((vendedor) => {
          // Skip if there is already a meeting between these two
          if (existingPairs.has(`${comprador.id}_${vendedor.id}`)) return null;
